...
import { createCategory, renameCategory } from './category-repository';

vi.mock('@/lib/supabase', () => ({ getSupabaseClient: vi.fn() }));

const duplicate = {
  code: '23505',
  message: 'duplicate key value violates unique constraint "account_categories_active_name_key"',
};
...
describe('finance repository errors', () => {
  ...
  it('preserves other database failures for shared error mapping', async () => {
    const databaseError = { code: '42501', message: 'permission denied' };
    mockClient({ error: databaseError });
    const failure = createCategory('account', 'Lazer');
    await expect(failure).rejects.toBe(databaseError);
  });
});
